...
class ContinuousFuturesProcessor:
    def __init__(self, full_ref_trd):
        self.full_ref_trd = full_ref_trd

    def near_exp_continuous_futures(self, code: str, n: int):
        ref_trd = self.full_ref_trd.query(f"fut_code == '{code}'").copy()
        if len(ref_trd) == 0:
        ...
        px_adj, roll_dates = self.hist_adj(ref, px)
        return px, px_adj, roll_dates

    def most_active_continuous_futures(self, code: str, n: int):
        ref_trd = self.full_ref_trd.query(f"fut_code == '{code}'").copy()
        if len(ref_trd) == 0:
            logger.warning(f'no trade & refs for future: {code}')
    ...
    def preprocess(ref: pd.DataFrame, trd: pd.DataFrame):
        trd['trade_date'] = pd.to_datetime(trd['trade_date'], format='%Y%m%d')
        ref['list_date'] = pd.to_datetime(ref['list_date'], format='%Y%m%d')
        ref['delist_date'] = pd.to_datetime(ref['delist_date'], format='%Y%m%d')
...
if __name__ == '__main__':
    ...
    processor = ContinuousFuturesProcessor(ContinuousFuturesProcessor.preprocess(fut_ref, fut_trd))

    if_px, if_px_adj, if_roll_dates = processor.near_exp_continuous_futures('IF', n=3)
    p_px, p_px_adj, p_roll_dates = processor.most_active_continuous_futures('P', n=3)
